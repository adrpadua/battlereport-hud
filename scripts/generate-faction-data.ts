...
async function main() {
  ...
  console.log(`\n📦 Parsing ${catalogues.length} catalogues...\n`);
  ...
  for (const { factionId, filename: originalFilename, content: xmlContent } of catalogues) {
    try {
      ...
      if (faction.units.length === 0) {
        console.log(`  ⏭️  Skipping ${faction.name} (no units found)`);
        continue;
      }
      ...
      console.log(`  ✅ ${faction.name}: ${faction.units.length} units`);
...
export async function load_${varName}(): Promise<FactionData> {
  ...
  return data.default as FactionData;
      ...
      console.warn(`  ⚠️  Failed to parse ${factionId}:`, error instanceof Error ? error.message : error);
    }
  }
...
`;
  ...
  console.log(`\n✨ Generated data for ${factionIndex.factions.length} factions`);
